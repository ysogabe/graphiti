...
def _build_neo4j_fulltext_query(
    ...
    # Parenthesise the group filter. Without it, precedence turns
    # `group_id:"a" OR group_id:"b" AND (query)` into
    # `group_id:"a" OR (group_id:"b" AND query)` — the first group then matches
    # unconditionally and the leg returns query-independent facts (measured: five
    # different queries returned the same handful of rows).
    if group_ids_filter:
